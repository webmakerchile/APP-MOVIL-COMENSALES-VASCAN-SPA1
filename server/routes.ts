...
export async function registerRoutes(app: Express): Promise<Server> {
    ...
    try {
      ...
      for (const casino of casinosList) {
        ...
        for (const f of fechasToProcess) {
          ...
          if (!minuta) continue;
          // Asegura que el staff (interlocutor/encargado) tenga pedido para esta
          // fecha aunque nadie haya abierto Gestión Diaria ese día.
          await ensureStaffPedidosForDate(casino.id, f, [minuta]);
  ...
  app.get("/api/reportes/inscripcion-detalle", requireAdmin, async (req: Request, res: Response) => {
    try {
      ...
      // Filtramos por la fecha de SERVICIO de la minuta (m.fecha), no por
      // createdAt. El staff (interlocutor/encargado) puede tener su pedido
      // creado retroactivamente, por lo que createdAt no reflejaría el rango.
      const filtered = allPedidos.filter(p => {
        const m = minutaById.get(p.minutaId);
        if (!m) return false;
  ...
  app.get("/api/totem/update-package", async (req: Request, res: Response) => {
    ...
    // Incluir paquete completo de better-sqlite3 con binario Windows
    // Estrategia: copiar el paquete a temp, reemplazar binario Linux por Windows, archivar
    const bsqlSrc = path.join(nmDir, "better-sqlite3");
    const bsqlTmp = path.join(totemTmpDir, "nm", "better-sqlite3");
    if (fs.existsSync(bsqlSrc)) {
      try {
        // Copiar todo el paquete JS al dir temporal usando fs.cpSync
        fs.mkdirSync(bsqlTmp, { recursive: true });
        fs.cpSync(bsqlSrc, bsqlTmp, { recursive: true });
        // Reemplazar el binario Linux con el de Windows
        if (sqliteNodePath) {
          const winBinDest = path.join(bsqlTmp, "build", "Release", "better_sqlite3.node");
          fs.mkdirSync(path.dirname(winBinDest), { recursive: true });
          fs.copyFileSync(sqliteNodePath, winBinDest);
        }
