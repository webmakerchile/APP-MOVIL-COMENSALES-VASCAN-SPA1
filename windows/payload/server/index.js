...
var DatabaseStorage = class {
  ...
  async getUserByRut(rut) {
    const [user] = await db.select().from(users).where((0, import_drizzle_orm2.eq)(users.rut, rut));
    return user;
  }
  async getAllUsers() {
  ...
  async updateUser(id, data) {
    const [user] = await db.update(users).set(touch(data)).where((0, import_drizzle_orm2.eq)(users.id, id)).returning();
    return user;
  }
  async deleteUser(id) {
    const [u] = await db.update(users).set(tombstone()).where((0, import_drizzle_orm2.eq)(users.id, id)).returning();
    return !!u;
  }
  // ── Casinos ──
  async getCasinos() {
    return db.select().from(casinos).where((0, import_drizzle_orm2.eq)(casinos.activo, true));
  }
  async getAllCasinos() {
    return db.select().from(casinos);
  }
  async getCasino(id) {
    const [casino] = await db.select().from(casinos).where((0, import_drizzle_orm2.eq)(casinos.id, id));
    return casino;
  }
  ...
  async updateCasino(id, data) {
    const [casino] = await db.update(casinos).set(touch(data)).where((0, import_drizzle_orm2.eq)(casinos.id, id)).returning();
    return casino;
  }
  async deleteCasino(id) {
    const [casino] = await db.update(casinos).set(tombstone()).where((0, import_drizzle_orm2.eq)(casinos.id, id)).returning();
    return !!casino;
  }
  async hardDeleteCasino(id) {
    const [casino] = await db.delete(casinos).where((0, import_drizzle_orm2.eq)(casinos.id, id)).returning();
    return !!casino;
  }
  // ── Minutas ──
  async getMinutasByCasino(casinoId) {
    return db.select().from(minutas).where((0, import_drizzle_orm2.and)((0, import_drizzle_orm2.eq)(minutas.casinoId, casinoId), (0, import_drizzle_orm2.eq)(minutas.activo, true)));
  }
  async getAllMinutasByCasino(casinoId) {
    return db.select().from(minutas).where((0, import_drizzle_orm2.eq)(minutas.casinoId, casinoId));
  }
  async getAllMinutas() {
    return db.select().from(minutas);
  }
  async getMinuta(id) {
    const [minuta] = await db.select().from(minutas).where((0, import_drizzle_orm2.eq)(minutas.id, id));
    return minuta;
  }
  ...
  async updateMinuta(id, data) {
    const [minuta] = await db.update(minutas).set(touch(data)).where((0, import_drizzle_orm2.eq)(minutas.id, id)).returning();
    return minuta;
  }
  async deleteMinuta(id) {
    const [minuta] = await db.update(minutas).set(tombstone()).where((0, import_drizzle_orm2.eq)(minutas.id, id)).returning();
    return !!minuta;
  }
  // ── Pedidos ──
  async getAllPedidos() {
  ...
  async getPedidosByUser(userId) {
    return db.select().from(pedidos).where((0, import_drizzle_orm2.eq)(pedidos.userId, userId));
  }
  async getPedidoByUserAndMinuta(userId, minutaId) {
    const [pedido] = await db.select().from(pedidos).where((0, import_drizzle_orm2.and)((0, import_drizzle_orm2.eq)(pedidos.userId, userId), (0, import_drizzle_orm2.eq)(pedidos.minutaId, minutaId)));
    return pedido;
  }
  ...
  async updatePedido(id, data) {
    if (DB_MODE === "totem" && sqlite) {
      const tx = sqlite.transaction((d) => {
        const [p] = db.update(pedidos).set(d).where((0, import_drizzle_orm2.eq)(pedidos.id, id)).returning().all ? db.update(pedidos).set(d).where((0, import_drizzle_orm2.eq)(pedidos.id, id)).returning().all() : [];
    ...
    const [pedido] = await db.update(pedidos).set(touch(data)).where((0, import_drizzle_orm2.eq)(pedidos.id, id)).returning();
    return pedido;
  }
  async getPedidosByMinuta(minutaId) {
    return db.select().from(pedidos).where((0, import_drizzle_orm2.eq)(pedidos.minutaId, minutaId));
  }
  // ── Familias ──
  ...
  async updateFamilia(id, data) {
    const [familia] = await db.update(familias).set(touch(data)).where((0, import_drizzle_orm2.eq)(familias.id, id)).returning();
    return familia;
  }
  async deleteFamilia(id) {
    const [familia] = await db.update(familias).set(tombstone()).where((0, import_drizzle_orm2.eq)(familias.id, id)).returning();
    return !!familia;
  }
  // ── Periodos ──
  async getPeriodosByCasino(casinoId) {
    return db.select().from(periodos).where((0, import_drizzle_orm2.eq)(periodos.casinoId, casinoId));
  }
  async getAllPeriodos() {
    return db.select().from(periodos);
  }
  async getPeriodo(id) {
    const [periodo] = await db.select().from(periodos).where((0, import_drizzle_orm2.eq)(periodos.id, id));
    return periodo;
  }
  ...
  async updatePeriodo(id, data) {
    const [periodo] = await db.update(periodos).set(touch(data)).where((0, import_drizzle_orm2.eq)(periodos.id, id)).returning();
    return periodo;
  }
  async deletePeriodo(id) {
    const [periodo] = await db.update(periodos).set(tombstone()).where((0, import_drizzle_orm2.eq)(periodos.id, id)).returning();
    return !!periodo;
  }
};
...
var upload = (0, import_multer.default)({ dest: "/tmp/uploads/" });
...
async function registerRoutes(app2) {
  ...
  app2.put("/api/usuarios/:id", requireAdmin, async (req, res) => {
    try {
      ...
      if (newPwd) updateData.password = await import_bcryptjs2.default.hash(newPwd, 10);
...
function configureExpoAndLanding(app2) {
  ...
  app2.use(import_express.default.static(path3.resolve(process.cwd(), "public")));
